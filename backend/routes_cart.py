...
async def get_cart(user_id: str, request: Request):
    ...
    cart = await db.carts.aggregate(pipeline).to_list(length=1)
    if cart:
        return normalize_object_ids(cart[0])
    return {"user_id": user_id, "items": []}

@router.post("/cart")
async def update_cart(cart: Cart, request: Request):
    db = request.app.state.db
    payload = cart.dict(exclude_unset=True)
    ...
    await db.carts.update_one({"user_id": payload["user_id"]}, {"$set": payload}, upsert=True)
    return {"success": True}

# More endpoints (remove item, clear cart) can be added here
